...
export default class PicBedUploader extends Plugin {
    ...
    async onload() {
        ...
        this.addRibbonIcon('upload', 'PicBedUploader', (evt: MouseEvent) => {
            uploadImage();
        });
...
class ImageUploadModal extends Modal {
    ...
    private setupUploadArea(uploadArea: HTMLElement, imagePreview: HTMLImageElement, textSpan: HTMLSpanElement) {
        uploadArea.addEventListener('click', () => this.triggerFileInput(uploadArea, imagePreview, textSpan));
        ...
        uploadArea.addEventListener('drop', (event) => {
            ...
            if (event.dataTransfer) {
                this.handleFiles(event.dataTransfer.files, uploadArea, imagePreview, textSpan);
            }
        });
    }

    private triggerFileInput(uploadArea: HTMLElement, imagePreview: HTMLImageElement, textSpan: HTMLSpanElement) {
        const fileInput = this.createFileInput(uploadArea, imagePreview, textSpan);
        document.body.appendChild(fileInput);
        fileInput.click();
        document.body.removeChild(fileInput);
    }

    private createFileInput(uploadArea: HTMLElement, imagePreview: HTMLImageElement, textSpan: HTMLSpanElement): HTMLInputElement {
        ...
        fileInput.addEventListener('change', () => {
            this.handleFiles(fileInput.files, uploadArea, imagePreview, textSpan);
        });

        return fileInput
    }

    private handleFiles(files: FileList | null, uploadArea: HTMLElement, imagePreview: HTMLImageElement, textSpan: HTMLSpanElement) {
        if (files && files.length > 0) {
            ...
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = (e) => {
                    ...
                    imagePreview.onload = () => {
                        imagePreview.style.height = `${uploadArea.offsetHeight - 4}px`;
                    }
