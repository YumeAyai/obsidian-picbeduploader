...
class ImageUploadModal extends Modal {
	...
	constructor(app: App, private plugin: MyPlugin, private onSubmit: (name: string | null, description: string | null, result: string | ArrayBuffer | null) => void) {
		super(app);
		this.result = null;
		this.file = null;
		this.name = 'Figure'
		...
		this.loginEndpoint = this.plugin.settings.loginEndpoint;
		this.username = this.plugin.settings.username;
		this.password = this.plugin.settings.password;
		this.cookie = this.plugin.settings.cookie;
	...
	private async handleUpload() {
		if (this.file) {
			...
			new Notice(`cok: ${this.cookie}`);

			return fetch(this.uploadEndpoint, {
				method: 'POST',
				body: formData,
				// headers: {
				//     'Cookie': this.cookie || await this.getCookies()
				// }
			}).then(async response => {
				if (!response.ok) {
...
class SampleSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName('User Name')
			.setDesc('Enter your name. Attention: User name will be saved as plain text.')
			.addText(text => text
		...
		new Setting(containerEl)
			.setName('Password')
			.setDesc('Enter your password. Attention: Password will be saved as plain text.')
			.addText(text => text
